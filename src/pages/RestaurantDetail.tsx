...
import { reviewService, restaurantService, menuService } from '../services/firebaseService';
import { Review, Restaurant, MenuItem } from '../types';
...
const RestaurantDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Subscribe to restaurant data in real-time
    const unsubscribeRestaurant = restaurantService.subscribeToRestaurant(id, (resData) => {
      if (resData) setRestaurant(resData);
      setLoading(false);
    });
    
    // Load reviews once (reviews don't need real-time as much, but we keep them loaded)
    ...
    // Subscribe to menu in real-time
    const unsubscribeMenu = menuService.subscribeToMenuItems(id, (items) => {
      setMenu(items);
  ...
  if (loading) return (
    <div className="max-w-4xl mx-auto px-6 py-24 text-center">
      <div className="animate-pulse flex flex-col items-center">
        <div className="w-20 h-20 bg-zinc-100 dark:bg-zinc-900 rounded-[2.5rem] mb-6" />
        <div className="h-8 bg-zinc-100 dark:bg-zinc-900 w-64 rounded-xl mb-4" />
        <div className="h-4 bg-zinc-100 dark:bg-zinc-900 w-32 rounded-lg" />
      </div>
    </div>
  );
    ...
    <div className="max-w-4xl mx-auto px-6 py-10">
            ...
            <div>
              ...
              <div className="flex items-center gap-4 text-xs">
                <div className="flex items-center gap-1.5 text-green-400 font-black italic bg-white/10 px-2 py-1 rounded-lg backdrop-blur">
                  <Star size={14} fill="currentColor" /> {restaurant.rating || 0}
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <AnimatePresence mode="popLayout">
          {menuLoading ? (
            [1, 2, 3].map(i => (
              <div key={i} className="p-4 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl h-32 animate-pulse" />
            ))
            ...
            return (
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="text-xl font-black text-zinc-900 dark:text-white tracking-tighter italic">₹{item.price.toFixed(2)}</div>
                </div>
                ...
                <div className="relative shrink-0">
                  {item.image ? (
                    ...
                    <div className="w-20 h-20 rounded-xl bg-zinc-50 dark:bg-zinc-950 flex items-center justify-center text-zinc-300">
                      <Utensils size={24} />
                    </div>
                  )}
