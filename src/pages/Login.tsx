...
const Login: React.FC<LoginProps> = ({ theme, toggleTheme }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignup) {
        await registerWithEmail(formData.email, formData.password, formData.name, accountType);
      } else {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <AnimatePresence mode="wait">
              {isSignup && (
                ...
                >
                  <div>
                    <label className="text-[9px] font-black tracking-[0.2em] text-zinc-400 uppercase mb-2 block ml-4">Account Type</label>
                    <div className="flex bg-zinc-50 dark:bg-zinc-950 p-1.5 rounded-2xl border border-zinc-100 dark:border-zinc-800">
                      <button
                        type="button"
